<x-app-layout>
	...
	<div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8 bg-white my-10 rounded shadow-md">
		<form method="post" action="{{ route('events.update', $event->id) }}">
			...
			<div class="flex justify-center">
				<div class="mb-3 xl:w-96">
					...
					<x-buk-flat-pickr id="end_date" name="end_date" value="{{$event->end_date}}"
					                  class="w-full pl-4 pr-10 py-2 leading-none rounded-lg shadow-sm focus:outline-none focus:shadow-outline text-gray-600 font-medium "/>
				</div>
			</div>
      ...
      focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none" aria-label="Default select example"
						...
						<option selected value="{{$event->status->id}}">{{$event->status->name}}</option>
      ...
      focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none" aria-label="Default select example"
						...
						<option selected value="{{$event->asset->id}}">{{$event->asset->name}}</option>
						@foreach($assets as $asset)
							@if ($asset->id !== $event->asset->id)
			...
			<div class="flex justify-center">
				<div class="mb-3 xl:w-96">
					...
					<x-buk-input type="number"
					             ...
					             id="quantity" placeholder="Quantity" name="qty" value="{{$event->qty}}"/>
